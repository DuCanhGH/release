...
const createRelease = async (
    ...
    try {
        if (exists) {
            response = await githubConnection.rest.repos.updateRelease({
                ...body,
                body: changelog,
                release_id: exists,
            ...
            response = await githubConnection.rest.repos.createRelease(body);
        }
    } catch (err) {
        response = {};
    }
...
const checkReleaseStatus = async () => {
    ...
    if (!flags.showUrl) {
        try {
            open(releaseURL, { wait: false });
            logger.error(`${prefix}. Opened in browser...`);
    ...
    logger.error(`${prefix}: ${releaseURL}`);
    process.exit(1);
};
